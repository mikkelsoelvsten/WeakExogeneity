...
for is = 1:ns_m; 
  if includecode(is) ~= 0;          % Ignore series if includecode == 0
      ...
      %Include for disabling first and second differences of the data
      if take_differences~=1
          if tcode(is) == 2 || tcode(is) == 3
              tcode(is) = 1;
          elseif tcode(is) == 5 || tcode(is) == 6
              tcode(is) = 4;
          end
      end
      y=transx(xq,tcode(is));                  % Transform .. log, first difference, etc.
      
      if take_differences==2
          Y = y(12:end);
          X = [Y,ones(length(Y),1),y(4:end-8),y(3:end-9),y(2:end-10),y(1:end-11)];
          keep = sum(isnan(X),2)==0;
          Y = X(keep,1);
          X = X(keep,2:6);
          y2 = zeros(length(Y),1);
          y2(keep) = Y - X*((X'*X)\(X'*Y));
